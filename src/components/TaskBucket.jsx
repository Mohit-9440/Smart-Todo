...
const TaskBucket = ({
  ...
  return (
    <div
      className={`rounded-lg border ${bucketConfig.containerClasses} p-4 animate-fade-in`}
    >
      {/* Bucket Header */}
      ...
      {/* Bucket Content */}
      <div className='space-y-3'>{renderBucketContent()}</div>
    </div>
  );
};
...
const getBucketConfig = status => {
  const configs = {
    [TASK_STATUS.ONGOING]: {
      icon: Clock,
      containerClasses:
        'border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-900/20 bucket-ongoing',
    ...
    [TASK_STATUS.SUCCESS]: {
      icon: CheckCircle,
      containerClasses:
        'border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-900/20 bucket-success',
      emptyIcon: (
    ...
    [TASK_STATUS.FAILURE]: {
      icon: XCircle,
      containerClasses:
        'border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20 bucket-failure',
